...
[[https://github.com/milkypostman/powerline/issues/58#issuecomment-60395853][Workaround]] for [[https://github.com/milkypostman/powerline/issues/58][bad interaction]] between =powerline= and
=desktop-save-mode=:
...
This only seems to work for the fore-front frame. You have to manually
run =powerline-reset= on the other frames. Maybe this will work:

#+begin_src emacs-lisp
  (defun bjs/powerline-delete-cache ()
    (set-frame-parameter nil 'powerline-cache nil))

  (add-hook 'desktop-save-hook 'bjs/powerline-delete-cache)
#+end_src

* General Configuration
** Default Mode

[[http://www.gnu.org/software/emacs/manual/html_node/emacs/Init-Examples.html][Set the default mode to =text-mode=]]
...
#+begin_src emacs-lisp
(defun bjs/generate-tab-stops (&optional width max)
...
(setq tab-stop-list (bjs/generate-tab-stops))
#+end_src
...
#+begin_src emacs-lisp
  (use-package powerline
    :commands (powerline powerline-reset)
    :config
    (progn
...
#+begin_src emacs-lisp
  ...
  (use-package ibuffer
    ...
    :config
    (progn
      (add-hook 'ibuffer-hook 'bjs/ibuffer-vc-hook)))
...
#+begin_src emacs-lisp
    (use-package ido
      ...
      (progn
        (ido-mode t)
        (add-hook 'ido-setup-hook 'bjs/ido-keymap-mods)))
#+end_src
...
Use [[https://github.com/juanjux/evil-search-highlight-persist][evil-search-highlight-persist]] to get Vim's =hlsearch=
functionality.
...
#+begin_src emacs-lisp :tangle no
  (use-package evil-search-highlight-persist
    :ensure evil-search-highlight-persist
    :config (progn
              (global-evil-search-highlight-persist t)
              (evil-ex-define-cmd "noh[lsearch]" 'search-highlight-persist-remove-all)))
#+end_src

*** enew Command

I miss the =:enew= command. I want an easy way to create a new empty
buffer that doesn't necessarily have a filename tied to it. Someone
has already [[https://github.com/echristopherson/.emacs.d/blob/master/init.el#L117][re-implemented it in their .emacs]].
...
Vim binds =Y= to the same as =yy= (yank line). In Vim, this doesn't
match up with other sets of keystrokes (e.g. =dd= deletes the line,
=D= deletes to end of line, =cc= 'changes' the line, =C= 'changes' to
end of line). My =.vimrc= binds =Y= to 'yank to end of line.' Let's do
the same here. [[http://mattbriggs.net/blog/2012/02/27/awesome-emacs-plugins-evil-mode/][Someone already did]].
...
(define-key evil-normal-state-map (kbd "gc") #'bjs/evil-comment-dwim)
#+end_src
...
Bind =C-p= to =bjs/find-file= so that we can get =CtrlP=-like
...
(define-key evil-normal-state-map "\C-p" 'bjs/find-file)
#+end_src
...
Let's make sure that the standard =hjkl= movement keys work in many of
the standard magit modes.
...
Use [[https://github.com/joshwnj/json-mode][json-mode]], which extends =js-mode= to work better on json files.
...
#+begin_src emacs-lisp
  ...
  (use-package python-mode
    ...
    :config
    (progn
      (add-hook 'python-mode-hook 'bjs/python-mode-prog-mode-fix)
      (add-hook 'python-mode-hook 'bjs/python-mode-indentation)
      (add-hook 'python-mode-hook 'bjs/pyvenv-autoload)
...
#+begin_src emacs-lisp
  (defun bjs/python-mode-indentation ()
...
#+begin_src emacs-lisp
  ...
  (defun bjs/python-mode-prog-mode-fix ()
...
#+begin_src emacs-lisp
  ...
  (defun bjs/pyvenv-autoload ()
    (interactive)
    (message "bjs/pyvenv-autoload")
    (if (projectile-project-p)
        (bjs/pyvenv-load-project-venv)
      (bjs/pyvenv-load-local-venv)))

  (defun bjs/pyvenv-load-local-venv ()
    (interactive)
    (message "bjs/pyvenv-load-local-venv")
    (let ((venv (expand-file-name ".venv"
                                  (file-name-directory (buffer-file-name)))))
      (if (file-accessible-directory-p venv)
          (bjs/pyvenv-from-file venv))))

  (defun bjs/pyvenv-load-project-venv ()
    (interactive)
    (message "bjs/pyvenv-load-project-venv")
    (let* ((project-root (projectile-project-root))
           (venv (expand-file-name ".venv" project-root)))
      (if (file-accessible-directory-p venv)
          (bjs/pyvenv-from-file venv))))

  (defun bjs/pyvenv-from-file (venv-file)
    (let ((venv-name (bjs/read-file-to-string venv-file)))
      (message "Loading virtualenv: %s" venv-name)
      (pyvenv-workon venv-name)))

  (defun bjs/read-file-to-string (file)
    (with-temp-buffer
...
#+begin_src emacs-lisp
  (use-package less-css-mode
    ...
    (progn
      ...
      (add-hook 'less-css-mode-hook 'color-identifiers-mode)
...
=scss-compile-at-save= makes more sense to me as part of a
=scss-mode-hook= that tries to determine the build process for a
source file, then locally (per-buffer) configures the build command,
and turns on =scss-compile-at-save=. I feel like turning this feature
on by default is a dubious design choice.
...
#+begin_src emacs-lisp
  (use-package js2-mode
    :defer 1
    :mode ("\\.js\\'" . js2-mode)
    :pre-load (setq-default js2-auto-indent-p t)
    :config
    (progn
      (add-hook 'js2-mode-hook 'bjs/js2-process-environment)
      ;; (add-hook 'js2-mode-hook 'bjs/nodejs-modules-check)
      (add-hook 'js2-mode-hook 'bjs/turn-on-flycheck-mode-js2)
...
#+begin_src emacs-lisp
  ;; (defun bjs/nodejs-modules-check ()
...
#+begin_src emacs-lisp
  (defun bjs/js2-process-environment ()
...
#+begin_src emacs-lisp
  (defun bjs/turn-on-flycheck-mode-js2 ()
...
Turn on =sh-mode= for =*.zsh= and =*.zsh-theme= files:
...
Trigger =sh-mode= to recognize =*.zsh= and =*.zsh-theme= files as
z-shell:
...
#+begin_src emacs-lisp
  ...
  (add-hook 'sh-mode-hook 'bjs/zsh-hook)
#+end_src

*Sources*: [[http://stackoverflow.com/questions/20558402/open-zsh-scripts-in-sh-mode-in-emacs][#1]]
...
#+begin_src emacs-lisp
  (use-package ssh-config-mode
    ...
    :mode ((".ssh/config\\'" . ssh-config-mode)
           ...
           ("authorized_keys2?\\'" . ssh-config-mode)))
#+end_src
...
Section for employer-specific stuff.

** Sowingo

#+begin_src emacs-lisp
  (defun sowingo/js2-mode-setup ()
...
- http://emacsrookie.com/2011/09/13/do-you-remember/
- http://sachachua.com/blog/2010/09/new-note-taking-workflow-with-emacs-org-mode/

** TODO Getting Flycheck working w/ python + virtualenv

- [[https://github.com/flycheck/flycheck/issues/312][Flycheck Github: Open Issue for :cwd property]]
- [[https://github.com/rejeep/f.el][File Functions]]
- [[http://www.lunaryorn.com/2013/11/26/syntax-checker-executables-in-flycheck.html][Syntax Checker Executables in Flycheck]]
- [[https://github.com/flycheck/flycheck/pull/272][Flycheck Github: Override Executables]]

** TODO Read up on YASnippet

http://capitaomorte.github.io/yasnippet/snippet-expansion.html

** TODO magit + squashing commits

http://howardism.org/Technical/Emacs/magit-squashing.html

** TODO Checkout emacs-ansible

https://github.com/k1LoW/emacs-ansible/blob/master/ansible.el
...
(add-hook 'guile-hook 'bjs/guile-setup)
#+end_src
...
- Blog Posts
  - [[http://mattbriggs.net/blog/2012/02/27/awesome-emacs-plugins-evil-mode/][Awesome Emacs Plugins: Evil Mode (mattbriggs.net)]]

- Projects
  - pyvenv: [[https://github.com/jorgenschaefer/pyvenv][github]]

- Discussions
  - [[http://www.reddit.com/r/emacs/comments/2qhoh0/emacsd_organization/][.emacs.d organization]] ([[http://www.reddit.com/r/emacs/][/r/emacs]])

- Screencasts
  - [[https://www.youtube.com/watch?v=hbmV1bnQ-i0][Handmade Hero Emacs Setup]]
